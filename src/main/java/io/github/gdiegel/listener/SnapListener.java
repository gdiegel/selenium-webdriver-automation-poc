...
public class SnapListener extends AbstractWebDriverEventListener {
  ...
  private String getCurrentTestPath() {
    return format("%s.%s.%03d", getContext().getRequiredTestClass().getSimpleName(),
        getContext().getRequiredTestMethod().getName(), getLongAdder().intValue());
  }

  @Override
  public void beforeFindBy(By by, WebElement element, WebDriver driver) {
    getLongAdder().increment();
    LOG.debug("[{}] [{}] beforeFindBy {}", getCurrentTestPath(), driver.getCurrentUrl(),
        by.toString());
  ...
  public void beforeClickOn(WebElement element, WebDriver driver) {
    getLongAdder().increment();
    LOG.debug("[{}] [{}] beforeClickOn {}", getCurrentTestPath(), driver.getCurrentUrl(),
        element.toString());
    saveScreenshot(driver, getCurrentTestPath());
    savePageSource(driver, getCurrentTestPath());
  }
  ...
  public void afterClickOn(WebElement element, WebDriver driver) {
    getLongAdder().increment();
    LOG.debug("[{}] [{}] afterClickOn {}", getCurrentTestPath(), driver.getCurrentUrl(),
        element.toString());
    saveScreenshot(driver, getCurrentTestPath());
    savePageSource(driver, getCurrentTestPath());
  }
  ...
  public void beforeChangeValueOf(WebElement element, WebDriver driver, CharSequence[] keysToSend) {
    getLongAdder().increment();
    LOG.debug("[{}] [{}] beforeChangeValueOf {} to {}", getCurrentTestPath(),
        driver.getCurrentUrl(), element.toString(), keysToSend);
    saveScreenshot(driver, getCurrentTestPath());
    savePageSource(driver, getCurrentTestPath());
  }
  ...
  public void afterChangeValueOf(WebElement element, WebDriver driver, CharSequence[] keysToSend) {
    getLongAdder().increment();
    LOG.debug("[{}] [{}] afterChangeValueOf {} to {}", getCurrentTestPath(), driver.getCurrentUrl(),
        element.toString(), keysToSend);
    saveScreenshot(driver, getCurrentTestPath());
    savePageSource(driver, getCurrentTestPath());
  }
  ...
  private void saveScreenshot(WebDriver webdriver, String identifier) {
    ...
    final File destFile = new File(
        format("./snaps/%s/%s.png", now.format(ofPattern("yyyyMMdd'T'HHmm")), identifier));
    try {
  ...
  private void savePageSource(WebDriver webdriver, String identifier) {
    final String srcFile = webdriver.getPageSource();
    final File destFile = new File(
        format("./snaps/%s/%s.html", now.format(ofPattern("yyyyMMdd'T'HHmm")), identifier));
    try (final ByteArrayInputStream bais = new ByteArrayInputStream(srcFile.getBytes(UTF_8))) {
